...
from dockerfiles.cuda_python.images import get_target_image as get_target_image_cuda_python, get_cuda_tag, get_cuda_config
...
def get_target_image(config: dict) -> str | None:
    ...
    uv_image = f"uv:{config['uv_version']}"
...
def get_config(image: str) -> dict:
    ...
    uv_version = parsed_image["images_infos"][1]["image_tag"]
...
def get_dependency(target_image: str) -> str:
    ...
    return get_target_image_cuda_python({
...
def get_build_args(target_image: str) -> dict:
    ...
    build_args["UV_VERSION"] = config["uv_version"]
