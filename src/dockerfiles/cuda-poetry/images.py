...
def get_target_image(config: dict) -> str | None:
    ...
    return f"{docker_user}/{image_name}:{get_cuda_tag(config)}--python-{python_version}--poetry-{poetry_version}{target_suffix}"
...
def get_config(target_image: str) -> dict:
    ...
    return {
        ...
        **get_cuda_config(cuda_tag),
        ...
        "poetry_version": meta_details[2].split("-")[1],
    }
...
def build(target_image: str) -> None:
    ...
    build_args["POETRY_VERSION"] = f"{config['poetry_version']}.*"
